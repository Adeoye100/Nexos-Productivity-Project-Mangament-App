...
export function HabitTracker() {
  ...
  const getLevel = useCallback((ds: string): 0 | 1 | 2 | 3 | 4 => {
    ...
    if (!data) return 0
    return intensityLevel(data.count, Math.max(habits.length, 1))
  ...
  const handleAddHabit = () => {
    ...
    addHabit({ name: newName.trim(), category: newCategory.trim() || undefined, targetFrequency: newFreq })
    setNewName(""); setNewCategory(""); setNewFreq("daily"); setShowAddForm(false)
  }
                    ...
                    return (
                      <div
                        ...
                        style={{
                          width:           CELL,
                          height:          CELL,
                          backgroundColor: cell.date ? `var(--hm-${level})` : "transparent",
                        }}
